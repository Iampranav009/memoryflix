...
export async function GET(request: Request) {
  try {
    ...
    // 4. Determine Hero Banner element (the featured season, or fallback to the latest)
    const hero = seasons.find((s: any) => s.featured) || (seasons.length > 0 ? seasons[seasons.length - 1] : null);
